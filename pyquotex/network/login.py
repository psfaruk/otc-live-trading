...
class Login(Browser):
    ...
    async def get_settings(self) -> tuple[Any, dict[str, Any] | None]:
        # window.settings is NOT always in the first script tag — search all.
        self.ssid = None
        settings_data: dict = {}
        for script in self.get_soup().find_all("script", {"type": "text/javascript"}):
            txt = script.get_text()
            if "window.settings" not in txt:
                continue
            match = re.sub(
        ...
        # Fallback: if window.settings had no token, try the digest API.
        if not self.ssid:
            try:
                digest_resp = await self.send_request(
                    "GET",
        ...
        return self.response, settings_data or None
